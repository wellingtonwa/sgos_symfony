		...
		foreach($this->dadosModulo as $dadosModulo){
			...
			foreach(array_keys($this->dadosCandidatos[$dadosModulo->numero_inscricao]['modulos']) as $nomeModulo){
				...
				foreach($matchs[0] AS $idx=>$nota){
					...
					$notaString = trim(str_replace('</td>', '', str_replace('<td  align="right" >', '', $nota)));
					
					
					if(!str_replace('*', '', $notaString)){
						$notaNumerica = 0;
					}
					else{
						$notaNumerica = str_replace(',', '.', $notaString);
					}
					
					$this->dadosCandidatos[$dadosModulo->numero_inscricao]['modulos'][$nomeModulo]['disciplinas'][$arrayDisciplinas[$contadorDisciplinas-1]][$tipoProva] = $notaString;
					$this->dadosCandidatos[$dadosModulo->numero_inscricao]['modulos'][$nomeModulo]['disciplinas'][$arrayDisciplinas[$contadorDisciplinas-1]][$tipoProva.'Num'] = $notaNumerica;
					unset($matchs[0][$idx]);
				}
			}
		...
		foreach($this->dadosCandidatos as $dadosCandidato){
			foreach($dadosCandidato['modulos'] as $modulo){
				foreach($modulo['disciplinas'] as $codDisciplina => $disciplina){
					$somaNotas = $disciplina['notaDiscursivaNum']+$disciplina['notaObjetivaNum'];
...
<html>
	<head>
		<META HTTP-EQUIV="Refresh" CONTENT="0;URL=http://sgospen.localhost/manutencaoDadosPismMaisRecenteAinda2.php">
	</head>
